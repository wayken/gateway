package cloud.apposs.gateway.plugin.runner.limit.action;
...
public class LimitBlockAction implements Action {
    ...
    public LimitBlockAction(Param parameters) {
        this.code = parameters.getInt(LimitConstant.Action.KEY_CODE, 403);
        this.responseType = parameters.getString(WafConstant.Action.KEY_RESPONSE);
        this.content = parameters.getString(LimitConstant.Action.KEY_CONTENT, "");
    }
    ...
    public String handleResponseContentGenerator() throws Exception {
        if (responseType == null) {
            ...
            InputStream pageInputStream = getClass().getClassLoader().getResourceAsStream("pages/overlimit.html");
