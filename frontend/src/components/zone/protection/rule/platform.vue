<template>
  <div class="a-zone-protection-rule platform inline-flex-c-n-n">
    ...
    <div class="content inline-flex-c-n-n">
      <div class="table">
        <el-table :data="dataList">
          ...
          <el-table-column prop="name" :label="$t('protection.rule.rule-name')" width="auto" min-width="180">
            <template #default="{ row }">
              <div class="name inline-pointer" @click="handleView(row)">{{ row.name || '-' }}</div>
            </template>
          </el-table-column>
          <el-table-column prop="action" :label="$t('protection.rule.rule-action')" width="220">
            <template #default="{ row }">
              <div class="action inline-flex-r-c-n"
                :style="{
                  backgroundColor: getCommonActionTypeInfo(row.action.type)?.color
                }"
              >
                <el-icon>
                  <component :is="getCommonActionTypeInfo(row.action.type)?.icon"></component>
                </el-icon>
                <div class="name" v-if="row.action.type">
                  {{ $t('rule.action.type.' + row.action.type) }}
                </div>
              </div>
            </template>
          </el-table-column>
          <el-table-column prop="priority" :label="$t('extension.priority')" width="220">
            <template #default="{ row }">
...
const handleLoad = () => {
  ...
  ioload('zone', 'loadWafRuleList', params).then((result) => {
...
const handleAdd = () => {
  ...
  router.push({ path: `/zone/protection/rule/add/${zone}` })
}
const handleEdit = (data: any) => {
  const zone = router.currentRoute.value.params.id
  const id = data.id
  router.push({ path: `/zone/protection/rule/edit/${zone}/${id}` })
}
const handleDelete = (data: any) => {
  ...
  iopost('zone', 'xhrWafRuleRemove', params, {
    onMessage: true
  }).then(() => {
    handleLoad()
  })
...
const handleDataUpdate = (data: any) => {
  ...
  iopost('zone', 'xhrWafRuleUpdate', data, {
    onMessage: true
  }).then(() => {
    handleLoad()
  })
...
const handleExport = (data: any) => {
  ...
  useFileDownload(dataJson, `Rule_${data.id}_${formattedDate}.json`)
}
</script>
