<template>
  <div class="a-zone-protection-limit--edit a-zone-common-form platform inline-flex-c-n-n">
    <div class="head inline-flex-r-c-n">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item :to="{ path: `/zone/protection/${$route.params.id}` }">
          {{ $t('menu.protection') }}
        </el-breadcrumb-item>
        <el-breadcrumb-item :to="{ path: `/zone/protection/limit/${$route.params.id}` }">
          {{ $t('submenu.protection.limit') }}
        </el-breadcrumb-item>
        <el-breadcrumb-item>
      ...
      >
        ...
        <div class="card">
          <div class="title">
            ...
            <div class="name">{{ $t('protection.limit.limit-config') }}</div>
          </div>
          <a-limit-config ref="limitConfigRef" />
        </div>
        <div class="card">
          <div class="title">
          ...
          <a-rule-action ref="ruleActionRef" blockable />
        </div>
      </el-form>
    </div>
...
import ALimitConfig from '@/components/zone/protection/limit/config/index.vue'
...
const limitConfigRef = ref<InstanceType<typeof ALimitConfig>>()
...
const handleLoad = async () => {
  ...
  result = await ioload('zone', 'loadLimitRuleInfo', params)
  Object.keys(result).forEach((key) => {
    loadFormData[key] = result[key]
  })
  ruleMvelRef.value?.handleExpressionLoad(loadFormData.rule)
  limitConfigRef.value?.handleConfigLoad({
    characteristics: result.resources,
    burst: result.burst,
    burstUnit: result.burstUnit
  })
...
const handleSubmit = async (deploy: boolean) => {
  ...
  validate = await limitConfigRef.value?.handleFormValidate()
  if (!validate) {
    return
  }
  ...
  formRef.value.validate((valid: boolean) => {
    ...
    const route = router.currentRoute.value
    const limitConfig = limitConfigRef.value?.handleConfigRead()
    const params: any = {
      id: route.params.rid,
      zone: route.params.id,
