<template>
  <div class="a-zone-cache-rule platform inline-flex-c-n-n">
          ...
          <el-table-column prop="action" :label="$t('protection.rule.rule-action')" width="220">
            <template #default="{ row }">
              <div class="action inline-flex-r-c-n"
                :style="{
                  backgroundColor: getCacheActionTypeInfo(row.action.type)?.color
                }"
              >
                <el-icon>
                  <component :is="getCacheActionTypeInfo(row.action.type)?.icon"></component>
                </el-icon>
...
import { getCacheActionTypeInfo } from '@/components/common/rule/rule'
...
const handleEdit = (data: any) => {
  const zone = router.currentRoute.value.params.id
  const id = data.id
  router.push({ path: `/zone/cache/rule/edit/${zone}/${id}` })
}
const handleDelete = (data: any) => {
  ...
  iopost('zone', 'xhrCacheRuleRemove', params, {
    onMessage: true
  }).then(() => {
    handleLoad()
  })
...
const handleDataUpdate = (data: any) => {
  ...
  iopost('zone', 'xhrCacheRuleUpdate', data, {
    onMessage: true
  }).then(() => {
    handleLoad()
  })
...
const handleExport = (data: any) => {
  ...
  useFileDownload(dataJson, `Cache_${data.id}_${formattedDate}.json`)
}
</script>
