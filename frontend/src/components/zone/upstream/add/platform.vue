<template>
  <div class="a-upstream--add a-zone-common-form platform inline-flex-c-n-n">
    ...
    <div class="content">
      ...
      >
        <div class="card">
          ...
          <el-form-item v-if="loadFormData.type === 'dns'" :label="$t('upstream.dns-domain')" prop="domain"
            :rules="{
              required: true,
              trigger: 'change',
...
const { iopost, ioload, progression } = useRequest()
...
const handleProviderLoad = () => {
  ioload('provider', 'loadProviderList', null).then((result) => {
    ...
    loadFormData.ai.parameters.model = loadProviderModelList.value.length > 0 ? loadProviderModelList.value[0] : ''
  })
}
const handleNodeAdd = () => {
...
const handleSubmit = () => {
  formRef.value.validate((valid: boolean) => {
    ...
    iopost('zone', 'xhrUpstreamAdd', params, {
      onMessage: true
    }).then(() => {
