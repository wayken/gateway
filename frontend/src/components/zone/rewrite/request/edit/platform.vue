<template>
  <div class="a-zone-rewrite-request--edit a-zone-common-form platform inline-flex-c-n-n">
    <div class="head inline-flex-r-c-n">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item :to="{ path: `/zone/rewrite/${$route.params.id}` }">
          {{ $t('menu.rewrite') }}
        </el-breadcrumb-item>
        <el-breadcrumb-item :to="{ path: `/zone/rewrite/request/${$route.params.id}` }">
          {{ $t('submenu.rewrite.request') }}
        </el-breadcrumb-item>
        <el-breadcrumb-item>
...
const handleLoad = async () => {
  ...
  result = await ioload('zone', 'loadRewriteRequestRuleInfo', params)
  Object.keys(result).forEach((key) => {
    loadFormData[key] = result[key]
  })
  ruleMvelRef.value?.handleExpressionLoad(loadFormData.rule)
  rewriteActionRef.value?.handleValueLoad(loadFormData.action)
}
...
const handleSubmit = async (deploy: boolean) => {
  ...
  formRef.value.validate((valid: boolean) => {
    ...
    iopost('zone', 'xhrRewriteRequestUpdate', params, {
      ...
      router.push({ path: `/zone/rewrite/request/${zone}` })
    })
  })
}
const handleBack = () => {
