<template>
  <div class="a-zone-rewrite-redirect platform inline-flex-c-n-n">
...
const handleLoad = () => {
  ...
  ioload('zone', 'loadRewriteRedirectRuleList', params).then((result) => {
...
const handleAdd = () => {
  ...
  router.push({ path: `/zone/rewrite/redirect/add/${zone}` })
}
const handleEdit = (data: any) => {
  const zone = router.currentRoute.value.params.id
  const id = data.id
  router.push({ path: `/zone/rewrite/redirect/edit/${zone}/${id}` })
}
const handleDelete = (data: any) => {
  ...
  iopost('zone', 'xhrRewriteRedirectRemove', params).then(() => {
    handleLoad()
  })
}
const handleDataUpdate = (data: any) => {
  data.zone = router.currentRoute.value.params.id
  iopost('zone', 'xhrRewriteRedirectUpdate', data, {
    onMessage: true
  }).then(() => {
    handleLoad()
  })
...
const handleExport = (data: any) => {
  ...
  useFileDownload(dataJson, `Rewrite_${data.id}_${formattedDate}.json`)
}
</script>
