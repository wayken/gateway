<template>
  <div class="a-zone-rewrite-response platform inline-flex-c-n-n">
...
const handleLoad = () => {
  ...
  ioload('zone', 'loadRewriteResponseRuleList', params).then((result) => {
...
const handleAdd = () => {
  ...
  router.push({ path: `/zone/rewrite/response/add/${zone}` })
}
const handleEdit = (data: any) => {
  const zone = router.currentRoute.value.params.id
  const id = data.id
  router.push({ path: `/zone/rewrite/response/edit/${zone}/${id}` })
}
const handleDelete = (data: any) => {
  ...
  iopost('zone', 'xhrRewriteResponseRemove', params).then(() => {
    handleLoad()
  })
}
const handleDataUpdate = (data: any) => {
  data.zone = router.currentRoute.value.params.id
  iopost('zone', 'xhrRewriteResponseUpdate', data, {
    onMessage: true
  }).then(() => {
    handleLoad()
  })
...
const handleStatusUpdate = (data: any, status: number) => {
  ...
  iopost('zone', 'xhrRewriteResponseUpdate', params, {
    onMessage: true
  }).then(() => {
    handleLoad()
  })
...
const handleExport = (data: any) => {
  ...
  useFileDownload(dataJson, `Response_${data.id}_${formattedDate}.json`)
}
</script>
