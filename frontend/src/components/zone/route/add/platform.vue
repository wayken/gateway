<template>
  <div class="a-route--add a-zone-common-form platform inline-flex-c-n-n">
          ...
          <template v-if="loadFormData.upstream.type === 'echo'">
            ...
            >
              <el-row :gutter="20" style="width: 100%;">
                ...
                <el-col :span="6">
                  <el-input-number v-model="loadFormData.upstream.code" :min="0" controls-position="right"
...
const handleProviderLoad = () => {
  ioload('provider', 'loadProviderList', null).then((result) => {
    ...
    loadFormData.upstream.ai.provider = result.length > 0 ? result[0].id : ''
    loadFormData.upstream.ai.parameters.model = loadProviderModelList.value.length > 0 ? loadProviderModelList.value[0] : ''
  })
}
...
const handleUpstreamNodeValidator = (rule: any, value: any, callback: any) => {
  if (value.host === '') {
    ...
    callback(new Error(i18n.t('extension.please-input-placeholder') + i18n.t('upstream.schema')))
  } else {
    callback()
  }
}
  ...
  formRef.value.validate((valid: boolean) => {
    ...
    if (loadFormData.upstreamId === 'manual') {
      ...
      if (upstreamType === 'echo') {
        params.upstream.code = loadFormData.upstream.code
    ...
    iopost('zone', 'xhrRouteAdd', params).then(() => {
