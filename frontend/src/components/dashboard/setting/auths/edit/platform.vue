<template>
  <el-dialog draggable width="720px" append-to-body class="a-setting-auths--edit platform"
...
const handleLoad = () => {
  ...
  ioload('auth', 'loadAuthInfo', params).then((result) => {
...
const handleSubmit = () => {
  formRef.value.validate((valid: boolean) => {
    ...
    iopost('auth', 'xhrAuthUpdate', params, {
      onMessage: true
    }).then((result) => {
      params.id = result.id
