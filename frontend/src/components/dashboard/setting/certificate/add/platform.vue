<template>
  <el-dialog draggable width="720px" append-to-body class="a-setting-certificate--add platform"
    :title="$t('setting.add-certificate')"
    :model-value="visible"
    @opened="handleOpen"
    @close="handleClose"
  >
    ...
    >
      ...
      <el-form-item :label="$t('extension.remark')" prop="remark">
        <el-input type="textarea" :rows="4" v-model="loadFormData.remark" />
      </el-form-item>
    </el-form>
    <template #footer>
      <span class="footer">
...
const handleSubmit = () => {
  formRef.value.validate((valid: boolean) => {
    ...
    iopost('certificate', 'xhrCertificateAdd', params, {
      onMessage: true
    }).then((result) => {
      params.id = result.id
