<template>
  <el-dialog draggable width="720px" append-to-body class="a-setting-ips--edit platform"
...
const handleLoad = () => {
  ...
  ioload('ip', 'loadIpInfo', params).then((result) => {
    Object.keys(result).forEach((key) => {
      if (key === 'cidrs') {
        loadFormData['content'] = result.cidrs.join('\n')
        return
      }
